...
export const updateOrCreateUserFromOauth = async ({ oauthUserInfo }) => {
  const { id: googleId, verified_email: isVerified, email } = oauthUserInfo; // Is the "email" stored like this?

  const db = getDbConnection("react-auth");
  const existingUser = await db.collection("users").findOne({ email });

  if (existingUser) {
    // If user exists, add data to the existing user
    ...
    const result = await db
      .collection("users")
      .insertOne({ email, googleId, isVerified, info: {} /* ... */ }); // create new user
